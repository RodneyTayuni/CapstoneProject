    ...
    <a href="" id="logoutLink"><i class="fas fa-sign-out-alt"></i> Log Out</a>
        ...
        <div class="main_content">
            <div class="Registration_container_form">
                <h2 class="Registration_container_form_title">ADMIN/TELLER REGISTRATION FORM</h2>
                <form id="SignUp_DI" method="post" enctype="multipart/form-data" onsubmit="return validateFormCheck()">
                    <br>
                    <h3>Personal Details</h3><br>
                    <div class="User_Full_Name">
                        <!-- Last Name -->
                        <input type="text" name="DI_LastName" placeholder="&nbsp Last Name" pattern="[A-Za-z]+[A-Za-z ]*" title="Should only contain letters and not be empty" maxlength="25" oninput="processSpaces(this)" required>
                        
                        <!-- First Name -->
                        <input type="text" name="DI_FirstName" placeholder="&nbsp First Name" pattern="[A-Za-z]+[A-Za-z ]*" title="Should only contain letters and not be empty" maxlength="25" oninput="processSpaces(this)" required>
                        
                        <!-- Middle Name -->
                        <input type="text" name="DI_MiddleName" placeholder="&nbsp Middle Name" pattern="[A-Za-z]+[A-Za-z ]*" title="Should only contain letters and not be empty" maxlength="25" oninput="processSpaces(this)">
                        
                        <!-- Suffix -->
                        <input type="text" name="DI_Suffix" placeholder="&nbsp Suffix (optional)" pattern="[A-Za-z]+[A-Za-z ]*" title="Should only contain letters and not be empty" maxlength="25" oninput="processSpaces(this)">

                    </div>
                    <div class="row2">
                        <span class="exp_date" style="font-size: 20px;">Birthdate:</span>

                        <input type="date" name="DI_Birthdate" id="DI_birthdate" placeholder="&nbsp;Birthdate" max="2005-06-20"
                            style="margin: 0 0 0 1%;" required>

                       <select name="DI_CivilStatus" id="DI_civil-status" style="margin: 0 0 0 1%;" required>
        <option value="" selected hidden>Choose Civil Status</option>
        ...
        <option value="Divorced">Divorced</option>
                        ...
                        <select name="AdminRole" id="AdminRole" style="margin: 0 0 0 1%;" required>
        <option value="" selected hidden>Choose Admin Role</option>
        <option value="Admin">Admin</option>
        <option value="Teller">Teller</option>
    </select>
                        
                        
                        <select name="SEX" id="SexAdmin" style="margin: 0 0 0 1%;" required>
        <option value="" selected hidden>Choose Gender</option>
        <option value="MALE">MALE</option>
        <option value="FEMALE">FEMALE</option>
    </select>
                        <br>
                        <input type="text" name="DI_ContactNumber" placeholder="&nbsp Contact Number" pattern="09\d{9}"
       ...
       pattern="^(?=\s*\S)(?=.*[A-Za-z0-9])[A-Za-z0-9,-.\s]{5,}$"
       ...
       maxlength="50" required>
                       ...
                       <input type="text" name="DI_Citizenship" placeholder="&nbsp;Citizenship" pattern="[A-Za-z]{5,}"
       title="Should only contain letters and have at least 5 characters No white space"
       ...
       maxlength="10" required>
                    ...
                    <div class="button_Container_Registration">
                        <input type="submit" name="DI_Submit" class="DI_enrollment_btn" value="Add Admin">
...
function validateFormCheck() {
            ...
            var adminRoleSelect = document.getElementById('AdminRole');
            var selectedAdminRole = adminRoleSelect.value;

            var sexSelect = document.getElementById('SexAdmin');
            var selectedSex = sexSelect.value;

            // Check if the selected values are the default ("Choose Civil Status", "Choose Admin Role", or "Choose Gender")
            if (selectedCivilStatus === "" || selectedCivilStatus === 'Choose Civil Status') {
                alert("Please select a valid civil status.");
                return false;
            }

            if (selectedAdminRole === "" || selectedAdminRole === 'Choose Admin Role') {
                alert("Please select a valid admin role.");
                return false;
            }

            if (selectedSex === "" || selectedSex === 'Choose Gender') {
                alert("Please select a valid gender.");
                return false;
            }

            // Add additional validation or actions as needed
...
function processSpaces(input) {
    ...
    var inputs = $(this).find('input');
      ...
      if (isValid) {
        ...
        $.ajax({
          url: "../Admin/php_scripts/Admin_check_existingAdmin.php",
          type: "POST",
          data: formData,
          contentType: false,
          processData: false,
  ...
  $.ajax({
    url: "./php_scripts/Admin_Adding_Teller_Admin.php",
    type: "POST",
    data: formData,
    contentType: false,
    processData: false,
    success: function (response) {
      console.log("Admin Adding: " + response);

      // Handle success logic here without redirecting
      if (response.trim() === "success") {
        clearInputFields(); // Clear input fields if needed
