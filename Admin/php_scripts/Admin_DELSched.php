...
<?php
    include "../conn.php";
    $userdatadel = $_GET['DELID'] ?? '';

    $stmt_Sched = $conn->prepare("SELECT * FROM u896821908_bts.pdc_schedule WHERE PDC_SchedID = :DELID");
    ...
    if ($_SERVER["REQUEST_METHOD"] === "POST") {
        ...
        if (isset($_POST["Confirm_Del"]) && !empty($_POST["Confirm_Del"])) {
            ...
            $stmt_student = $conn->prepare("DELETE FROM u896821908_bts.pdc_schedule WHERE PDC_SchedID = :DELID");
