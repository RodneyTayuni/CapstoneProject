...
$userdataAdmin = isset($_GET['Admin']) ? $_GET['Admin'] : '';

$stmt = $conn->prepare("SELECT * FROM u896821908_bts.admin WHERE idadmin = :studentUsername");
$stmt->bindParam(':studentUsername', $userdataAdmin); // Replace $studentId with the actual student ID
$stmt->execute();
...
<body>
	<div id="AdminEditProf" class="modal_SignUp">
		<!-- Modal content -->
		<div class="modal-content_Signup">
			<div class="modal-header_SignUp">
				<span class="Adminclose_SignUp close_SignUp">&times;</span>
				<h1>Update Admin</h1>
				<br>
			</div>
			<div class="modal-body_SignUp">
				<div class="container">
                <form id="SignUp_Admin" method="post" enctype="multipart/form-data">
                        <div class="Personal Data">
                            ...
                            <div class="fields">
                                ...
                                <div class="second_row_input">
                                    ...
                                    <input type="date" name="Birthdate" id="Adminbirthdate" placeholder="&nbsp Birthdate"
                                        max="2005-06-20" value="<?php echo $birthdate; ?>" required>
                                    <select name="CivilStatus" id="Admincivil-status" class="civ_stat">
       ...
       class="Address" pattern="[A-Za-z0-9,-. ]{3,}" title="Should contain at least 3 characters, and may include letters, numbers, and special characters"
                                        ...
                                        value="<?php echo $completeAddress; ?>" required>
       ...
       pattern="[A-Za-z ]{3,}" title="Should contain at least 3 letters"
                        ...
                        <input type="hidden" name="CurrentEmail" id="AdmincurrentEmailInput" value="<?php echo $emailAddress; ?>">

                        <input type="email" name="EmailAdd" id="AdminemailInput" placeholder="&nbsp;Email Address"
            ...
            <input type="hidden" name="AdminID" placeholder="&nbsp New Username" class="username" maxlength="10" value="<?php echo $userdataAdmin; ?>" required>
                           ...
                           <div class="password-container">
                                <input type="password" name="pass" id="Adminpassword" class="password-input" placeholder="&nbsp; New Password" value="<?php echo $password; ?>">
                                <span class="toggle-password" onclick="togglePassword('#Adminpassword')"><i class="fas fa-eye"></i></span>
                            </div>
                            
                            <div class="password-container">
                                <input type="password" id="AdminconfirmPassword" class="password-input" value="<?php echo $password; ?>" placeholder="&nbsp; Confirm New Password">
                                <span class="toggle-password" onclick="togglePassword('#AdminconfirmPassword')"><i class="fas fa-eye"></i></span>
                            </div>
...
<script>
    function togglePassword(inputId) {
        ...
        if (input.type === 'password') {
            input.type = 'text';
            icon.className = 'fas fa-eye-slash'; /* Change the icon to eye slash */
        } else {
            input.type = 'password';
            icon.className = 'fas fa-eye'; /* Change the icon back to eye */
        }
    }
</script>
<script>
  ...
  var AdminconfirmPasswordInput = document.getElementById("AdminconfirmPassword");

  AdminpasswordInput.addEventListener("input", validateAdminPassword);
  AdminconfirmPasswordInput.addEventListener("input", validateAdminPassword);

  function validateAdminPassword() {
    var Adminpassword = passwordInput.value;
    var AdminconfirmPassword = confirmPasswordInput.value;
    ...
    if (regexPattern.test(Adminpassword) && regexPattern.test(AdminconfirmPassword)) {
  ...
  $(document).ready(function () {
    ...
    $("#SignUp_Admin").submit(function (event) {
  ...
  if ((password !== '' || AdminconfirmPassword !== '') && (!isLowerCaseValid || !isUpperCaseValid || !isNumberValid || !isLengthValid)) {
    alert("Password does not meet the requirements");
    isValid = false;
  }

  if (isValid) {
var formData = new FormData($("#SignUp_Admin").get(0));

    // Get the current email from the hidden input
    var currentEmail = $('#admincurrentEmailInput').val();
  ...
  formData.append('Username', username);
    ...
    $.ajax({
      url: "./php_scripts/admin_Staffiles/Admin_Edit/check_existingStaff_Enrolled.php",
      type: "POST",
...
function formSubmit(formData) {
  var submitButton = $('.Submit');
  $.ajax({
    url: "./php_scripts/admin_Staffiles/Admin_Edit/AdminEdit_NEnrollStaff_update.php?email=" + encodeURIComponent(formData.get('EmailAdd')),
    type: "POST",
    data: formData,
    contentType: false,
    processData: false,
