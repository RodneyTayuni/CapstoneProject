...
//AGE get Data chart
$queryAgeStudent = "SELECT idStudent, Firstname, Lastname, Birthdate FROM student WHERE ( (`PDC-MOTOR` IS NOT NULL AND TRIM(`PDC-MOTOR`) <> '') OR 
...
while ($rowAgeStudent = $stmtAgeStudent->fetch(PDO::FETCH_ASSOC)) {
    ...
    if ($age >= 18 && $age <= 21) {
        ...
        $ageCategory = "38 and above";
        $ageCounts[5]++;
    }

    // Store age category and count in the ageData array
    $ageData[] = array("category" => $ageCategory, "count" => $ageCounts);
}
...
$CountTDC_PDC = "SELECT * FROM student
    WHERE ( (`PDC-MOTOR` IS NOT NULL AND TRIM(`PDC-MOTOR`) <> '') OR 
(`PDC-CAR` IS NOT NULL AND TRIM(`PDC-CAR`) <> '') )
    and TDC IS NOT NULL and TDC_Cert_approve IS NULL or PDC_Cert_approve IS NULL and Enroll_Status = 'enrolled'";
$stmtCountTDC_PDC = $conn->prepare($CountTDC_PDC);
$stmtCountTDC_PDC->execute();
$resultsTDC_PDC = $stmtCountTDC_PDC->fetchAll(PDO::FETCH_ASSOC);

$CountTDC = 0;
$CountPDC = 0;
$CourseLabels = array();

foreach ($resultsTDC_PDC as $rowTDC_PDC) {
    if ($rowTDC_PDC['TDC'] === 'Enrolling' && $rowTDC_PDC['TDC_Cert_approve'] === NULL) {
        $CountTDC++;
    }

    if ($rowTDC_PDC['PDC-MOTOR'] !== NULL && trim($rowTDC_PDC['PDC-MOTOR']) !== '') {
        $CountPDC++;
    } elseif ($rowTDC_PDC['PDC-CAR'] !== NULL && trim($rowTDC_PDC['PDC-CAR']) !== '') {
        $CountPDC++;
    }
}

// Populate CourseLabels array
$CourseLabels[] = 'TDC';
$CourseLabels[] = 'PDC';



$CountGender = "SELECT Sex, COUNT(*) as count FROM student WHERE ( (`PDC-MOTOR` IS NOT NULL AND TRIM(`PDC-MOTOR`) <> '') OR 
        (`PDC-CAR` IS NOT NULL AND TRIM(`PDC-CAR`) <> '') )
and TDC IS NOT NULL and TDC_Cert_approve IS NULL or PDC_Cert_approve IS NULL and Enroll_Status = 'enrolled' GROUP BY Sex";
$stmtCountGender = $conn->prepare($CountGender);
$stmtCountGender->execute();

// Fetch the results
$resultsCountGender = $stmtCountGender->fetchAll(PDO::FETCH_ASSOC);

// Initialize variables to store counts
$CountMale = 0;
$CountFemale = 0;

foreach ($resultsCountGender as $rowCountGender) {
    if ($rowCountGender['Sex'] === 'MALE') {
        $CountMale = $rowCountGender['count']; // Fix: Use $rowCountGender instead of $row
    } elseif ($rowCountGender['Sex'] === 'FEMALE') {
        $CountFemale = $rowCountGender['count']; // Fix: Use $rowCountGender instead of $row
    }
}

//City get DataChart
$queryStdCity = "SELECT City, IFNULL(COUNT(*), 0) AS StudentCount FROM student WHERE ( (`PDC-MOTOR` IS NOT NULL AND TRIM(`PDC-MOTOR`) <> '') OR 
        (`PDC-CAR` IS NOT NULL AND TRIM(`PDC-CAR`) <> '') )
and TDC IS NOT NULL and TDC_Cert_approve IS NULL or PDC_Cert_approve IS NULL and Enroll_Status = 'enrolled' GROUP BY City";
...
$queryDistinctCities = "SELECT DISTINCT City FROM student WHERE ( (`PDC-MOTOR` IS NOT NULL AND TRIM(`PDC-MOTOR`) <> '') OR 
        (`PDC-CAR` IS NOT NULL AND TRIM(`PDC-CAR`) <> '') )
and TDC IS NOT NULL and TDC_Cert_approve IS NULL or PDC_Cert_approve IS NULL and Enroll_Status = 'enrolled'";
...
$queryGender = "SELECT Sex, COUNT(*) AS GenderCount FROM student WHERE ( (`PDC-MOTOR` IS NOT NULL AND TRIM(`PDC-MOTOR`) <> '') OR 
        (`PDC-CAR` IS NOT NULL AND TRIM(`PDC-CAR`) <> '') )
and TDC IS NOT NULL and TDC_Cert_approve IS NULL or PDC_Cert_approve IS NULL and Enroll_Status = 'enrolled' GROUP BY Sex";
...
<body>
    <div class="Container_nav">
        ...
        <div class="main_content">
            <header style="margin-bottom:1%;"><h2>TOTAL STUDENTS</h2></header>
            <div class="con1">
            <div class="students">
                <div class="male">
...
?>
             ...
             <div class="main_content" style="margin-top:-3%;">
            <header style="margin-bottom:1%;"><h2>COURSE ENROLLED STUDENTS</h2></header>
            <div class="con1">
            <div class="students">
                <div class="male">
                </div>
            ...
            <div class="students">
                ...
                <div class="label">
                    <h2>PDC</h2>
                    <label><?php echo $CountPDC ?></label>
                    </div>
                
            </div>
            </div>
            <div class="Course_chart">
                                    <canvas id="CourseChart" class = "ChartCourse"></canvas>
            </div>
...
var ctxCity = document.getElementById('addressChart').getContext('2d');
    ...
    //STD Course
var ctxSTDCourse = document.getElementById('CourseChart').getContext('2d');
var CourseChart = new Chart(ctxSTDCourse, {
    type: 'horizontalBar', // Use horizontal bar chart type
    data: {
        labels: ['TDC','PDC'], // Empty labels array
        datasets: [{
            data: [<?php echo $CountTDC; ?>, <?php echo $CountPDC; ?>],
            backgroundColor: ['green', 'orange'], // Color for each category
            borderWidth: 1,
        }]
    },
    options: {
        ...
        legend: { // Add legend to indicate colors
            display: true,
            labels: {
                fontColor: 'black',
                generateLabels: function(chart) {
                    var labels = [
                        {text: 'TDC', fillStyle: 'green'},
                        {text: 'PDC', fillStyle: 'orange'}
                    ];
                    return labels;
                }
            }
